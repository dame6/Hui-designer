...
<body class="body1">
    ...
    <nav class="navbar navbar-expand-md  ">
      ...
      <div class="collapse navbar-collapse " id="menuhamburger">
        <ul class="navbar-nav navbar-collapse ">
           <li class="nav-item">
            <a class="nav-link active hd" href="{{ url('/acceuil_admin') }}" >Acceuil</a>
          </li>
          <li class="nav-item">
            <a class="nav-link hd" href="{{ url('/articles') }}" >Article</a>
          </li>
          <li class="nav-item">
  ...
  <div class="container">
   <div >
     <div class="img-ad-01" style="background-image: url('img/group-2.png');">
      <h2 class="artad-1">Les Messages et Les Postulants pour devenir Membres</h2>
     </div>
 ...
 <section class="article  mb-5" >
                                  ...
                                  <tr>
                                      ...
                                      <td>
                                          <div class="form-button-action">
                                              <a href="{{ url('/suppmsg/'.$message->id) }}" class="btn btn-link btn-danger action" 
    ...
    <section class="article  mt-5" >
      <div class="container">
        <h2 class="parle-1">Postulants</h2>
        <div class="row justify-content-center">
          <div class="table-responsive">
                          <table id="add-row" class="display table table-striped table-hover" >
                              <thead>
                                  <tr>
                                      <th>Prenom Postulant</th>
                                      <th>Nom Postulant</th>
                                      <th>Mail </th>
                                      <th>Activité</th>
                                      <th style="width: 10%">Action</th>
                              ...
                              <tbody>
                                @foreach($communautes as $communaute)
                                  <tr>
                                       <td>{{ $communaute->prenom }} </td>
                                       <td>{{ $communaute->nom}}</td>
                                       <td>{{ $communaute->email  }} </td>  
                                      <td>{{ $communaute->activite }}</td>
                                      <td>
                                          <div class="form-button-action">
                                              <a href="{{ url('/suppcom/'.$communaute->id) }}" class="btn btn-link btn-danger action" 
                                                > <i class="fa fa-times">Supprimer</i></a> 
                                          </div>
                                      </td>
                                  </tr>
